...
function items(app, models, socketListener) {
    ...
    */
     ...
     * Buat Item
     */
    router.post('/', requiredPost(["name", "quantity", "unit", "price"]), a(async (req, res) => {
        // Ambil model
        const { Item } = models;

        // Variabel
        let { name, quantity, unit, price } = req.body;

        // Buat Item
        let data = await Item.create({ name, quantity, unit, price });
     ...
     * Update Item
     */
    router.put('/:id', requiredPost(["name", "quantity", "unit", "price"]), a(async (req, res) => {
        // Ambil model
        const { Item } = models;
        ...
        if (data) {
            // Update Item
            data = await data.update({ name, quantity, unit, price });
            // Response
            res.setStatus(res.OK);
            res.setData(data);
            res.go();
